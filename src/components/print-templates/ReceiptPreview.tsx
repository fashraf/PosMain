...
const DEMO_ITEMS = [
  { name: "Chicken Shawarma", qty: 2, price: 15.0, total: 30.0 },
  { name: "Arabic Coffee", qty: 3, price: 5.0, total: 15.0 },
  { name: "Kunafa", qty: 1, price: 12.0, total: 12.0 },
];
const SUBTOTAL = 57.0;
const DISCOUNT = 5.0;
const VAT_RATE = 0.15;
        ...
        <div className="bg-white rounded-lg shadow-sm mx-auto max-w-[280px] px-4 py-5 font-mono text-[11px] leading-relaxed space-y-2">
          ...
          {data.show_logo && (
            <div className={cn("flex", data.logo_position === "left" ? "justify-start" : data.logo_position === "right" ? "justify-end" : "justify-center")}>
              {data.logo_url ? (
                <img src={data.logo_url} alt="Logo" className="h-10 object-contain" />
              ) : (
                <div className="h-10 w-20 rounded bg-muted/60 flex items-center justify-center text-[9px] text-muted-foreground border border-dashed border-muted-foreground/30">LOGO</div>
              )}
            </div>
          )}
          ...
          <div className="text-center space-y-0.5">
            ...
            {data.show_cr_number && <p className="text-muted-foreground">CR# 1010XXXXXX</p>}
            {data.show_vat_number && <p className="text-muted-foreground">VAT# 3100XXXXXXXX03</p>}
          </div>
