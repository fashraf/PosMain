...
export default function StaffAttendanceList({ data }: Props) {
  return (
    <Card className="p-4 hover:shadow-md transition-shadow">
      <div className="flex items-center gap-2 mb-3">
        <UserCheck className="h-4 w-4" style={{ color: "#32c080" }} />
        <h3 className="text-sm font-semibold" style={{ color: "#2c8cb4" }}>Staff Attendance</h3>
      </div>
      {data.length === 0 ? (
            ...
            <tbody>
              {data.map((s, i) => (
                <tr key={i} className={`border-b last:border-0 hover:bg-muted/50 ${i % 2 === 0 ? "bg-muted/20" : ""}`}>
                  ...
                  <td className="py-1.5 text-muted-foreground">{s.employeeCode}</td>
