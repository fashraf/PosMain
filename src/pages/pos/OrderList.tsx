...
import { Plus, Search, Pencil, Banknote, XCircle, Trash2, AlertTriangle, ClipboardList, ChevronRight, ChevronDown } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="sticky top-0 z-30 bg-background border-b border-border px-5 py-3 flex items-center justify-between">
        ...
        <TouchButton className="rounded-xl h-10 px-4 gap-2" onClick={() => navigate("/pos")}>
          <Plus className="h-4 w-4" /> New Order
        </TouchButton>
      </div>

      {/* Stat Cards */}
      <OrderStatCards stats={stats || { total: 0, inProcess: 0, completed: 0, paymentPending: 0 }} isLoading={!stats} />

      {/* Tabs + Filters merged */}
      <div className="sticky top-[57px] z-20 bg-background border-b border-border px-5 py-2 flex flex-wrap items-center gap-2">
        <Tabs value={filters.tab} onValueChange={(v) => updateFilter("tab", v as OrderStatusTab)}>
          <TabsList className="bg-muted/50">
            <TabsTrigger value="all">All</TabsTrigger>
            <TabsTrigger value="active">Active</TabsTrigger>
            <TabsTrigger value="unpaid" className="gap-1">
              Unpaid <AlertTriangle className="h-3 w-3 text-destructive" />
            </TabsTrigger>
        ...
        <Select value={filters.dateFilter} onValueChange={(v) => updateFilter("dateFilter", v as DateFilter)}>
          <SelectTrigger className="w-[120px] h-9 rounded-lg text-sm"><SelectValue /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filters.orderType} onValueChange={(v) => updateFilter("orderType", v)}>
          <SelectTrigger className="w-[120px] h-9 rounded-lg text-sm"><SelectValue placeholder="Type" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filters.paymentMethod} onValueChange={(v) => updateFilter("paymentMethod", v)}>
          <SelectTrigger className="w-[120px] h-9 rounded-lg text-sm"><SelectValue placeholder="Payment" /></SelectTrigger>
          <SelectContent>
      ...
      <div className="flex-1 overflow-auto px-5 py-2">
        {isLoading ? (
          <div className="flex items-center justify-center h-40 text-muted-foreground text-sm">Loading orders…</div>
        ) : list.length === 0 ? (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[75px]">Payment</TableHead>
                    ...
                    >
                      ...
                      <TableCell>
                        <button
                          ...
                          onClick={(e) => { e.stopPropagation(); setDrawerOrder(order); }}
                        >
                          #{order.order_number}
                        </button>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
                          {!isCancelled && (
                            <button className="grid-action-btn" title="Edit Order"
                              onClick={() => navigate("/pos", { state: { editOrderId: order.id } })}>
                              <Pencil className="h-4 w-4" />
                            </button>
                          )}
                          {isUnpaid && (
                            <button className="grid-action-btn" title="Collect Payment"
                              onClick={() => setPayOrder(order)}>
                              <Banknote className="h-4 w-4" />
                            </button>
                          )}
                          {!isCancelled && (
                            <button className="grid-action-btn danger" title="Cancel Order"
                              onClick={() => setCancelOrder(order)}>
                              <XCircle className="h-4 w-4" />
                            </button>
                          )}
                          {isManager && (
                            <button className="grid-action-btn danger" title="Delete Order"
      ...
      {list.length > 0 && (
        <div className="sticky bottom-0 bg-background border-t border-border px-5 py-2 flex items-center justify-between text-sm">
          <span className="text-muted-foreground">Page {filters.page + 1}</span>
          <div className="flex gap-2">
            <TouchButton variant="outline" size="sm" className="rounded-lg"
      ...
      {/* Modals & Drawers */}
      <OrderDetailDrawer order={drawerOrder} open={!!drawerOrder} onOpenChange={(v) => !v && setDrawerOrder(null)} />
      {payOrder && (
