...
import { ArrowLeft, ArrowRight, Save, X, FileText, Package, DollarSign, Settings } from "lucide-react";
...
export default function IngredientsAdd() {
  ...
  const handleConfirmSave = () => {
    setIsSaving(true);
    setTimeout(() => {
      toast({ title: t("ingredients.addIngredient"), description: `${formData.name_en} has been added.` });
      setIsSaving(false);
  ...
  return (
    <div className="space-y-3 pb-20">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-semibold text-foreground">{t("ingredients.addIngredient")}</h1>
      </div>
      ...
      <div className={cn("fixed bottom-0 inset-x-0 bg-background border-t p-3 z-10", "flex items-center gap-3", isRTL ? "flex-row-reverse pe-[16rem] ps-4" : "ps-[16rem] pe-4")}>
        <div className={cn("flex-1 flex gap-2 justify-end", isRTL && "flex-row-reverse")}>
          <Button variant="outline" size="sm" onClick={() => navigate("/ingredients")} disabled={isSaving}>
