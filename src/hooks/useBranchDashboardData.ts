...
export function useBranchDashboardData(branchId: string | null) {
  ...
  const todayOrdersQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("pos_orders")
        .select("id, total_amount, subtotal, vat_amount, payment_status, payment_method, order_type, created_at, order_number")
  ...
  // Order items for category breakdown (today's orders)
  const orderItemsQuery = useQuery({
    queryKey: ["branch-dashboard-items", branchId],
    queryFn: async () => {
      if (!branchId) return [];
      const { from, to } = todayRange();
      // Get today's order IDs for this branch
  ...
  // Menu items lookup (for category mapping)
  ...
  todayPaid.forEach((o) => {
    ...
    paymentModeMap.set(method, (paymentModeMap.get(method) || 0) + 1);
  });
  ...
  const staffList: StaffListItem[] = staffData.map((s: any) => ({
    ...
    employeeCode: s.profiles?.employee_code || "—",
    isActive: s.profiles?.is_active ?? true,
  }));
  ...
  // Slowest orders (pending orders sorted by age)
  ...
  if (todayRevenue > yesterdayRevenue && yesterdayRevenue > 0) alerts.push({ type: "success", message: `Revenue up ${revChange}% vs yesterday!` });
